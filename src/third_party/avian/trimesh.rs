...
fn shape_to_trimesh(
    ...
    let vertices = vertices
        .into_iter()
        .map(|v| pos + (rot * Vec3::from(v)).f32())
        .collect();
    let indices = indices.into_iter().map(|i| i.into()).collect();
    ...
    compound.shapes().iter().fold(
        ...
        |(mut vertices, mut indices), (sub_pos, shape)| {
            ...
            indices.extend(
                inner_indices
                    .into_iter()
                    .map(|i| [i[0] + next_index, i[1] + next_index, i[2] + next_index]),
            );
            (vertices, indices)
        },
    )
